...
stats_format_xy = {
    UNIVERSAL: {
        ...
        # actually the entire stat number, using hog to separate each digit
        "DIGIT_WIDTH_RATIO": 60.0/1920.0,
...
def find_closest_nba_player_name(player_name_guess, player_height, all_players_dict):
    ...
    for player_name in all_players_dict.keys():
        ...
        player_first_name_length_diff = abs(
            len(player_name.split()[0]) - len(player_name_guess.split()[0]))
...
def preprocess_predict(img, clf, pp, save=False, save_path=False):
    ...
    pic_hog_fd = hog(pic_data.reshape((30, 30)), orientations=9, pixels_per_cell=(
    ...
    if save:
        cv2.imwrite(os.path.join(save_path, '%s_%s.png' %
                                 (prediction, id_generator())), save_data)
    return prediction
...
def get_name(img, player_height, card_format, save=False):
    ...
    for sharpen in range(0, 2):
        ...
        for y_adjust in range(0, 16):
            img_player_name = img.crop(calc_name_rect(
                width, height, y_adjust, card_format))
            if sharpen > 0:
            ...
            player_name_guess = re.sub(
                ' +', ' ', player_name_guess.replace('\n', ' ')).strip()
    ...
    if len(player_name_guess) == 0:
        if save:
            img.save('Training/Bad_%s_%s.png' %
                     (player_name_guess, id_generator()))
        return player_name_guess
    if player_name_guess not in all_players_dict:
        original_guess = player_name_guess
        player_name_guess = find_closest_nba_player_name(
            player_name_guess, player_height, all_players_dict)
    if save:
        img.save('Training/%s.png' % (player_name_guess))
    # Get original first and last name
    player_name = "%s %s" % (
        all_players_dict[player_name_guess]["firstName"], all_players_dict[player_name_guess]["lastName"])
    parse_logger.info("Original Name Guess: %s || Final Name %s" %
                      (original_player_name_guess, player_name))
    return player_name
...
def get_stat_rects(img_adv_stats):
    ...
    im_th = cv2.adaptiveThreshold(
        im_gray, 35, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 15, 2)
...
def get_stats(img, clf, pp, card_format, save=False, save_path='False'):
    ...
    for c in range(1, 3):
        for r in range(1, 9):
            ...
            width, height = img.size
            img_adv_stats = img.crop(calc_stat_rect(
                c, r, 0, width, height, card_format))
            digit = ""
            ...
            for rect in rects:
                ...
                if prediction == "bad":
                    continue
                # prevent double predictions of a single digit
                img_adv_stats.paste((0, 0, 0), (x1, y1, x2, y2))
            ...
            for d in range(1, 3):
                img_adv_stats = img.crop(calc_stat_rect(
                    c, r, d, width, height, card_format))
                rects = get_stat_rects(img_adv_stats)
                for rect in rects:
                    if not check_if_valid_stat_rect(rect, width, height):
                        continue
                    ...
                    roi = roi.convert('L')
                    roi = change_contrast(roi, 400)
                    prediction = preprocess_predict(
                        roi, clf, pp, save, save_path)
                    if prediction == "bad":
                        continue
                    # if card_stats["(%s,%s)"%(c,r)]["name"] == "Agility":
                    #   roi.show()
...
def get_card_img(img, card_format, save=False, save_path=False):
    ...
    img_card = img_card.resize(
        (int(round(img_card_width/1.5)), int(round(img_card_height/1.5))), Image.ANTIALIAS)
    output = StringIO()
...
def get_type(img, clf, pp, card_format, save=False, save_path=False):
    ...
    img.paste(0, [int(pos_rect[0]), int(pos_rect[1]),
                  int(pos_rect[2]), int(pos_rect[3])])
...
def parse_one(img, adv_stats_clf, adv_stats_pp, height_clf, height_pp, ovr_clf, ovr_pp, pos_clf, pos_pp, type_clf, type_pp, saveOption=False):
    ...
    card_dict["height"] = get_height(
        img, height_clf, height_pp, card_format, save=saveOption, save_path='Training/height_digits')
    card_dict["name"] = get_name(img, card_dict["height"], card_format)
    if len(card_dict["name"]) < 3:
        return card_dict
    card_dict["pos"] = get_pos(
        img, pos_clf, pos_pp, card_format, save=saveOption, save_path='Training/pos')
    card_dict["card_img"] = get_card_img(img, card_format)
    card_dict["type"] = get_type(
        img, type_clf, type_pp, card_format, save=saveOption, save_path='Training/type')
    card_dict["ovr"] = get_ovr(
        img, ovr_clf, ovr_pp, card_format, save=saveOption, save_path="Training/ovr_digits")
    card_dict["stats"] = get_stats(img, adv_stats_clf, adv_stats_pp,
                                   card_format, save=saveOption, save_path="Training/adv_stats_digits")
    card_dict["hash"] = create_hash(card_dict["pos"], card_dict["type"],
                                    card_dict["height"], card_dict["stats"], card_dict["name"], card_dict["ovr"])
    return card_dict
...
def parse_one_main_stats(img, ovr_clf, ovr_pp, pos_clf, pos_pp, type_clf, type_pp, height, saveOption=False):
    ...
    card_dict["pos"] = get_pos(
        img, pos_clf, pos_pp, card_format, save=False, save_path='Training/pos')
    card_dict["type"] = get_type(
        img, type_clf, type_pp, card_format, save=False, save_path='Training/type')
    card_dict["ovr"] = get_ovr(
        img, ovr_clf, ovr_pp, card_format, save=False, save_path="Training/ovr_digits")
    ...
    if parse_all:
        ...
        def thread_func_wrapper(img, adv_stats_clf, adv_stats_pp, height_clf, height_pp, ovr_clf, ovr_pp, pos_clf, pos_pp, type_clf, type_pp):
            card_dict = parse_one(img, adv_stats_clf, adv_stats_pp, height_clf,
                                  height_pp, ovr_clf, ovr_pp, pos_clf, pos_pp, type_clf, type_pp)
        ...
        for img_name in os.listdir("raw_cards"):
            ...
            request = threadpool.WorkRequest(thread_func_wrapper,
                                             ...
                                             [img_path, adv_stats_clf, adv_stats_pp, height_clf, height_pp,
                                                 ovr_clf, ovr_pp, pos_clf, pos_pp, type_clf, type_pp],
