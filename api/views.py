...
def parse_request(request):
    ...
    response = parse_one(img, adv_stats_clf, adv_stats_pp, height_clf,
                         height_pp, ovr_clf, ovr_pp, pos_clf, pos_pp, type_clf, type_pp, True)
    return HttpResponse(json.dumps(response))
...
def searchCardData(request):
    ...
    response = cards_db.find_one(
        {'hash': card_hash}, {'_id': False, 'card_img': False})
    return HttpResponse(json.dumps(response))
...
def searchCardImage(request):
    ...
    card_hash = request.GET.get('hash')
    response = cards_db.find_one(
        {'hash': card_hash}, {'_id': False, 'card_img': True})
